...
class StageViewController: UIViewController {
    ...
    @IBOutlet weak private var level1StackView: UIStackView!
    @IBOutlet weak private var level2StackView: UIStackView!
    @IBOutlet weak private var level3StackView: UIStackView!
    @IBOutlet weak private var level4StackView: UIStackView!
    @IBOutlet weak private var level5StackView: UIStackView!
    @IBOutlet weak private var level6StackView: UIStackView!
    @IBOutlet weak private var level7StackView: UIStackView!

    @IBOutlet weak private var bannerView: GADBannerView!

    private var allLevelLabel: [UILabel] {
        return [
            level1Label, level2Label, level3Label, level4Label, level5Label, level6Label, level7Label
        ]
    }
    ...
    private var allStackView: [UIStackView] {
        [
        level1StackView, level2StackView, level3StackView,
        level4StackView, level5StackView, level6StackView, level7StackView
        ]
    }
    ...
    required init?(coder: NSCoder, characterType: CharacterType) {
        ...
        self.totalCoins = CoinRepository.load() ?? 0
    ...
    private func configureButtonIsEnabled() {
        ...
        if totalCoins < questions[0].requiredCoins {
            filterButtons = allLevelButton
        } else if totalCoins < questions[1].requiredCoins {
            filterButtons = Array(allLevelButton[1...6])
        } else if  totalCoins < questions[2].requiredCoins {
            filterButtons = Array(allLevelButton[2...6])
        } else if  totalCoins < questions[3].requiredCoins {
            filterButtons = Array(allLevelButton[3...6])
        } else if  totalCoins < questions[4].requiredCoins {
            filterButtons = Array(allLevelButton[4...6])
        } else if  totalCoins < questions[5].requiredCoins {
            filterButtons = Array(allLevelButton[5...6])
        } else if  totalCoins < questions[6].requiredCoins {
    ...
    private func configureViewStackView() {
        allStackView.forEach { stackView in
            stackView.layer.cornerRadius = 10
            stackView.backgroundColor = UIColor(named: "stackViewBackground")
        }
    }
}
